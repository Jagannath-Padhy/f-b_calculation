import { CatalogData, Category, CustomGroupConfig, Customization, MenuItem, PriceRange } from '../types';
...
export class CatalogParser {
    ...
    return rawItems
      .filter(item => 
        item.tags?.some((t: any) => 
          t.list?.some((l: any) => l.code === 'type' && l.value === 'item')
        )
      )
      .map(item => ({
        id: item.id,
  ...
  static parseCustomizations(rawItems: any[]): Customization[] {
    return rawItems
      .filter(item => 
        item.tags?.some((t: any) => 
          t.list?.some((l: any) => l.code === 'type' && l.value === 'customization')
        )
      )
      .map(item => ({
        id: item.id,
  ...
  private static determineCategoryType(category: any): 'custom_group' | 'menu' {
    ...
    return typeTag?.list?.some((l: any) => l.value === 'custom_group') 
      ? 'custom_group' 
      : 'menu';
  }
  ...
  private static isDefaultSelection(item: any): boolean {
    const parentTag = item.tags?.find((t: any) => t.code === 'parent');
    return parentTag?.list?.some((l: any) => 
      l.code === 'default' && l.value === 'yes'
    ) || false;
  }
}